...
string labelFile = "/home/bhap/Desktop/yolov3/model_data/classesbdd.txt";
...
string videoFile = "/home/bhap/Documents/Video/test4.MP4";
...
void correct_box(Mat& img, float bbox[4], vector<int>& ltrb) {
    int l, r, t, b;
    ...
    ltrb.push_back(max(0, l));
    ltrb.push_back(max(0, t));
    ltrb.push_back(min(img.cols, r));
    ltrb.push_back(min(img.rows, b));
}
...
void nms(vector<Yolo::Detection> detections, vector<Yolo::Detection>& res, float nms_thresh = NMS_THRESH) {
    ...
    for (int i = 0; i < length; ++i) {
        if (detections[i].det_confidence * detections[i].class_confidence <= CONF_THRESH)
            continue;
        Yolo::Detection det;
...
bool onnxToTRTModel(const std::string& modelFile, const std::string& filename, IHostMemory*& trtModelStream) {
    ...
    // 等价于bulider.createNetwork(),通过Ibulider定义的名为creatNetwork()方法，创建INetwvector<Yolo::Detection> res;orkDefinition的对象，ntework这个指针指向这个对象
...
int main(int argc, char** argv) {
    ...
    Sort sort_class;
    sort_class.initialize_variable(MAX_AGE, MIN_HITS);
    ...
    while (1) {
        ...
        vector<vector<int>> yolo_dets;
        vector<vector<int>> out_sort;
        string predicted_class, label, id, score;
        for (size_t i = 0; i < res.size(); ++i) {
            vector<int> ltrb;
            correct_box(img, res[i].bbox, ltrb);
            // 跟踪目标类型暂选car、bus、truck
            if ((int)res[i].class_id == 0 || (int)res[i].class_id == 1 || (int)res[i].class_id == 4) {
                ltrb.push_back((int)res[i].class_id);
                yolo_dets.push_back(ltrb);
            }
            else {  // 画图-不参加跟踪的
                predicted_class = class_names[(int)res[i].class_id];
                score = format("%.2f", (res[i].det_confidence * res[i].class_confidence));
                // label = predicted_class + " " + score;
                label = predicted_class;
                rectangle(img, Point(ltrb[0], ltrb[1]), Point(ltrb[2], ltrb[3]), colors[(int)res[i].class_id], 2, 8);
                int l_w = label.length() * 7 + 30;
                int l_h = 11;
                rectangle(img, Point(ltrb[0], ltrb[1] - l_h), Point(ltrb[0] + l_w, ltrb[1]), colors[(int)res[i].class_id], -1, 8);
                putText(img, label, Point(ltrb[0], ltrb[1] - 1), FONT_HERSHEY_TRIPLEX, 0.5, Scalar(0, 0, 0), 1, 8);
            }
        }
        out_sort = sort_class.update(yolo_dets, count);

        for (size_t i = 0; i < out_sort.size(); ++i) {  // 画图-参加跟踪的
            predicted_class = class_names[out_sort[i][5]];
            id = format("id: %d", (out_sort[i][4]));
            label = predicted_class + " " + id;

            rectangle(img, Point(out_sort[i][0], out_sort[i][1]), Point(out_sort[i][2], out_sort[i][3]), colors[out_sort[i][4] % COLOR_NUM], 2, 8);
            int l_w = label.length() * 7 + 30;
            int l_h = 11;
            rectangle(img, Point(out_sort[i][0], out_sort[i][1] - l_h), Point(out_sort[i][0] + l_w, out_sort[i][1]), colors[out_sort[i][4] % COLOR_NUM], -1, 8);
            putText(img, label, Point(out_sort[i][0], out_sort[i][1] - 1), FONT_HERSHEY_TRIPLEX, 0.5, Scalar(0, 0, 0), 1, 8);
        }
        string label_fps;
